...
const WeatherIcon = ({ forecastDescription, city }) => {
  ...
  if (forecastDescription === "Heavy Intensity Rain" || forecastDescription === 'Moderate Rain') {
    return (
      <Wrapper>
  ...
  if (forecastDescription === "Clear Sky" || forecastDescription === 'Few Clouds') {
    return (
      <Wrapper>
  ...
  if (forecastDescription === "Broken Clouds") {
    return (
      <Wrapper>
        <TiWeatherPartlySunny style={{ color: "lightGray" }} className="icon" />
      </Wrapper>
    )
  }
